...
impl Player {
    ...
    pub fn projectiles(&self) -> List<Projectile> {
        self.inner().player.lock().unwrap().projectiles.clone()
    }

    /// The amount of money this Player has.
    pub fn money(&self) -> i64 {
        self.inner().player.lock().unwrap().money.clone()
    }

    /// The home base of the player.
    pub fn home_base(&self) -> Body {
        self.inner().player.lock().unwrap().home_base.clone()
    }

    /// The number of victory points the player has.
    pub fn victory_points(&self) -> i64 {
        self.inner().player.lock().unwrap().victory_points.clone()
    }
